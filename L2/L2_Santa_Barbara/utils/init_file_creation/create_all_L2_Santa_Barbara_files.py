...
def create_L2_Santa_Barbara_files(config_dir):
    ...
    if 1 in steps:
        ...
        import create_L2_Santa_Barbara_mitgrid as cm
    ...
    if 2 in steps:
        ...
        import create_L2_Santa_Barbara_bathymetry as cb
    ...
    if np.any(np.array(steps)>2):
        ...
        if not os.path.exists(grid_path):
            sys.exit("Need to make the grid netcdf file for this model for reference\n"
                     "   - Run the stitch_L2_Santa_Barbara_nc_grid_files_for_ref.py script with the -f 1 flag (and -r 150 -c 210)\n"
                     "   - Run the *for_grid model\n"
                     "   - Run the stitch_L2_Santa_Barbara_nc_grid_files_for_ref.py script")

    # step 3: make the initial conditions
    if 3 in steps:
        print('Step 3: Creating the pickup (initial conditions) file for the ' + L2_model_name + ' model')
        import create_L2_Santa_Barbara_pickup as cp
...
if __name__ == '__main__':
    ...
    create_L2_Santa_Barbara_files(config_dir)
