...
def split_mitgrid_file(config_dir,rows,cols,for_grid):
    ...
    mitgrid_file = os.path.join(config_dir,'L2','L2_Monterey_Bay','input','L2_Monterey_Bay.mitgrid')
    ...
    for ri in range(nr):
        for ci in range(nc):
            tile_subset = entire_grid[:,rows*ri:rows*(ri+1)+1,
                                        cols*ci:cols*(ci+1)+1]
            output_file = os.path.join(config_dir,'L2','L2_Monterey_Bay','input'+for_grid_suffix,'tile'+'{:03d}'.format(counter)+'.mitgrid')
