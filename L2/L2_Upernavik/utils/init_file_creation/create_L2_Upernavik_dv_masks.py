...
def create_dv_masks(config_dir, L2_model_name, print_level):
    ...
    for boundary in ['Upernavik_N','Ussing_Braeer','Kakivfaat','CTD','iceplume']:
        if print_level >= 1:
        ...
        n_row_tiles = int(np.shape(mask_grid)[0] / 25)
        n_col_tiles = int(np.shape(mask_grid)[1] / 25)

        max_N = 0
        for r in range(n_row_tiles):
            for c in range(n_col_tiles):
                # print(r*72, (r+1)*72, c*60, (c+1)*60)
                N = np.sum(mask_grid[r * 25:(r + 1) * 25, c * 25:(c + 1) * 25] != 0)
                if N > max_N:
                    max_N = N

        if print_level >= 2:
            print('        - The max points in any one tile is ' + str(max_N))
